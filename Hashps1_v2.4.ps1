...
function Export-SFVFile {
    ...
    try {
        $sfvContent = "; Generated by Hashps1 v2.4 on $([DateTime]::Now.ToString('yyyy-MM-dd HH:mm:ss'))`r`n"
...
"@
        ...
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Hashps1 v2.4").Show($toast)
    } catch {
...
function Initialize-TrayIcon {
    ...
    try {
        ...
        $script:notifyIcon.Text = "Hashps1 v2.4"
...
if ($script:PortableMode) {
    $form.Text = "Hashps1 v2.4  [WARNING: Portable Mode - No config/cache will be saved]"
} else {
    $form.Text = "Hashps1 v2.4"
}
...
$buttonImportBatchLog.Add_Click({
    ...
    if ($dialog.ShowDialog() -eq "OK") {
        try {
            ...
            $lines = $logContent -split "`r?`n" | Where-Object { $_ -match '\S' }

            $importedLines = @()
            foreach ($line in $lines) {
                # Use independent regex matches for each pattern
                $m1 = [regex]::Match($line, '^\s*(.+?):\s+((?:0x)?[a-fA-F0-9]{8,}|[A-Za-z0-9+/=]{16,})\s*$')
                if ($m1.Success) {
                    $filename = $m1.Groups[1].Value.Trim()
                    $hash = $m1.Groups[2].Value.Trim()
                    $importedLines += "$hash`t$filename"
                    continue
                }
                $m2 = [regex]::Match($line, '^\s*((?:0x)?[a-fA-F0-9]{8,}|[A-Za-z0-9+/=]{16,})\t(.+?)\s*$')
                if ($m2.Success) {
                    $hash = $m2.Groups[1].Value.Trim()
                    $filename = $m2.Groups[2].Value.Trim()
                    $importedLines += "$hash`t$filename"
                    continue
                }
                $m3 = [regex]::Match($line, '^\s*((?:0x)?[a-fA-F0-9]{8,}|[A-Za-z0-9+/=]{16,})\s+(.+?)\s*$')
                if ($m3.Success) {
                    $hash = $m3.Groups[1].Value.Trim()
                    $filename = $m3.Groups[2].Value.Trim()
                    $importedLines += "$hash`t$filename"
                    continue
                }
            }

            if ($importedLines.Count -eq 0) {
            ...
            [System.Windows.Forms.MessageBox]::Show(("Successfully imported {0} hash entries from batch log.`n`nClick 'Verify All' to verify hashes." -f $importedLines.Count), "Import Complete", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } catch {
...
$form.Add_Resize({
    if ($form.WindowState -eq 'Minimized' -and $checkMinimizeToTray.Checked) {
        ...
        if ($script:currentJobId -or $script:batchJobId) {
            $script:notifyIcon.BalloonTipTitle = "Hashps1 v2.4"
