...
function Export-SFVFile {
    ...
    try {
        $sfvContent = "; Generated by CrunchHash v2.6 on $([DateTime]::Now.ToString('yyyy-MM-dd HH:mm:ss'))`r`n"
...
"@
        ...
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("CrunchHash v2.6").Show($toast)
    } catch {
...
function Initialize-TrayIcon {
    ...
    try {
        ...
        $script:notifyIcon.Text = "CrunchHash v2.6"
...
if ($script:PortableMode) {
    $form.Text = "CrunchHash v2.6  [WARNING: Portable Mode - No config/cache will be saved]"
} else {
    $form.Text = "CrunchHash v2.6"
}
...
$labelHeader.Text = "CrunchHash v2.6"
...
$form.Add_Resize({
    if ($form.WindowState -eq 'Minimized' -and $checkMinimizeToTray.Checked) {
        ...
        if ($script:currentJobId -or $script:batchJobId) {
            $script:notifyIcon.BalloonTipTitle = "CrunchHash v2.6"
